...
- [ ] `WavCleanup` struct still exists in `whisper.rs` and is still registered immediately after the transient `.wav` is written (before the sidecar call)
- [ ] `TxtCleanup` struct still exists in `whisper.rs` and is still registered immediately after the sidecar call returns (before checking `output.status`)
- [ ] `chmod_0600_unix` is still called on both the transient `.wav` and the transcript `.txt` before either is read
- [ ] `cargo test --manifest-path src-tauri/Cargo.toml --locked` passes, specifically including `whisper::tests::wav_cleanup_removes_file_on_panic`, `whisper::tests::wav_cleanup_removes_file_on_drop`, `whisper::tests::txt_cleanup_removes_file_on_drop`, and `whisper::tests::txt_cleanup_ignores_missing_file`
- [ ] `scripts/check_log_phi.sh` passes clean (run it directly: `bash scripts/check_log_phi.sh`) — this is also enforced by the `log-phi-guard` CI job, but confirm it locally before a release cut too
- [ ] No new call site writes raw audio or transcript content to disk without an equivalent RAII cleanup guard (spot-check: `grep -rn "tokio::fs::write\|std::fs::write" src-tauri/src/*.rs` against the known-accounted-for list from the last full audit)
- [ ] If the whisper.cpp integration architecture changed since the last release (e.g., in-process binding, named pipes), re-open this document and re-assess — do not just re-check the boxes above unchanged

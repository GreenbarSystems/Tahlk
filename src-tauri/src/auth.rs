...
mod tests {
    ...
    fn clear_session_dek_removes_the_in_memory_key() {
        ...
        // audio_crypto's resolution-order test drives the same global, and
        // cargo runs them on different threads. If this clear() landed between
        // that test's set_session_dek_hex() and its audio_key(), the call fell
        // through to the OS keychain — which in CI is the D-Bus path that can
        // block indefinitely. So this is not only test hygiene: it was one
        // contributor to the intermittent CI hang.
